...
if(db2.has({key: `admin_${wolf.guild.id}`}) == true){
  ...
  if(!admins.includes(`id_${u.id}`)) return wolf.lineReply(`**❌ - <@!${u.id}> Is Not In Admins List .**`)
}else if(db2.has({key: `admin_${wolf.guild.id}`}) == false){

wolf.lineReply(`**❌ - <@!${u.id}> Is Not In Admins List .**`)
}
...
.setDescription(`**Done Remove <@!${u.id}> From Admins List ✅ .**`)
